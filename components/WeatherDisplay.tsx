...
export default function WeatherDisplay({
  ...
  if (!weatherData) {
    return (
      <div className={`${isDarkMode ? 'glass-strong bg-black/40' : 'bg-white/80 border border-slate-200 shadow-xl'} rounded-2xl p-6 w-full max-w-md ${isDarkMode ? 'backdrop-blur-md' : 'backdrop-blur-lg'}`}>
        <p className={`${isDarkMode ? 'text-white/60' : 'text-slate-600'} text-sm`}>{t.selectLocation}</p>
      </div>
    )
  }
  ...
  return (
    <div className={`${isDarkMode ? 'glass-strong bg-black/40' : 'bg-white/80 border border-slate-200 shadow-xl'} rounded-2xl p-6 w-full max-w-md ${isDarkMode ? 'backdrop-blur-md' : 'backdrop-blur-lg'} space-y-4`}>
      ...
      <div className="mb-4">
        {locationName && (
          <p className={`text-sm ${isDarkMode ? 'text-white/60' : 'text-slate-600'} mb-1`}>{locationName}</p>
        )}
        <div className="flex items-center gap-3 mb-2">
          <span className={`text-5xl font-bold ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{Math.round(current.temperature_2m)}°C</span>
          <div className="text-4xl">{weatherCode.emoji}</div>
        </div>
        <p className={`text-sm mb-1 ${isDarkMode ? 'text-white/60' : 'text-slate-600'}`}>{weatherCode.label}</p>
        <p className={`text-xs ${isDarkMode ? 'text-white/80' : 'text-slate-700'}`}>
      ...
      {(hasFog || hasHail || hasThunderstorm) && (
        <div className={`p-3 rounded-lg ${isDarkMode ? 'bg-yellow-500/20 border-yellow-500/30' : 'bg-yellow-50 border-yellow-200'}`}>
          <div className="flex items-center gap-2">
            {hasThunderstorm && <CloudLightning className={`w-4 h-4 ${isDarkMode ? 'text-yellow-300' : 'text-yellow-600'}`} />}
            {hasFog && <CloudFog className={`w-4 h-4 ${isDarkMode ? 'text-yellow-300' : 'text-yellow-600'}`} />}
            {hasHail && <Snowflake className={`w-4 h-4 ${isDarkMode ? 'text-yellow-300' : 'text-yellow-600'}`} />}
            <p className={`font-medium text-sm ${isDarkMode ? 'text-yellow-300' : 'text-yellow-700'}`}>
      ...
      {precipitation.type !== 'none' && (
        <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-blue-500/10 border-blue-500/20' : 'bg-blue-50 border-blue-200'}`}>
          <div className="flex items-center gap-3">
            {precipitation.type === 'rain' ? (
              <CloudRain className={`w-5 h-5 ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`} />
            ) : (
              <Snowflake className={`w-5 h-5 ${isDarkMode ? 'text-blue-300' : 'text-blue-500'}`} />
            )}
            <div>
      ...
      {/* Stats principales */}
      <div className="grid grid-cols-2 gap-4">
        <div className={`p-3 rounded-lg ${isDarkMode ? 'bg-white/5' : 'bg-slate-100/50 border border-slate-200'}`}>
          <div className="flex items-center gap-2 mb-1">
            <Droplets className={`w-4 h-4 ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`} />
            <span className={`text-xs ${isDarkMode ? 'text-white/60' : 'text-slate-600'}`}>{t.humidity}</span>
          </div>
          <p className={`text-lg font-semibold ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{current.relative_humidity_2m}{t.percent}</p>
        </div>

        <div className={`p-3 rounded-lg ${isDarkMode ? 'bg-white/5' : 'bg-slate-100/50 border border-slate-200'}`}>
          <div className="flex items-center gap-2 mb-1">
            <Gauge className={`w-4 h-4 ${isDarkMode ? 'text-purple-400' : 'text-purple-600'}`} />
            <span className={`text-xs ${isDarkMode ? 'text-white/60' : 'text-slate-600'}`}>{t.pressure}</span>
          </div>
          <p className={`text-lg font-semibold ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{Math.round(current.surface_pressure)} {t.hpa}</p>
        </div>
      </div>

      {/* Vent avec flèche pivotante */}
      <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-white/5' : 'bg-slate-100/50 border border-slate-200'}`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Wind className={`w-5 h-5 ${isDarkMode ? 'text-cyan-400' : 'text-cyan-600'}`} />
            <div>
              <p className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{t.wind}</p>
              <p className={`text-xs ${isDarkMode ? 'text-white/60' : 'text-slate-600'}`}>{windSpeedKmh} {t.kmh}</p>
            </div>
          </div>
          <div
      ...
      {current.visibility > 0 && (
        <div className={`p-3 rounded-lg ${isDarkMode ? 'bg-white/5' : 'bg-slate-100/50 border border-slate-200'}`}>
          <div className="flex items-center gap-2">
            ...
            <span className={`text-xs ${isDarkMode ? 'text-white/60' : 'text-slate-600'}`}>{t.visibility}</span>
            <span className={`ml-auto text-sm font-medium ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
        ...
        <div className={`pt-4 border-t ${isDarkMode ? 'border-white/10' : 'border-slate-200'}`}>
          <div className={`p-4 rounded-lg border ${aqiInfo.bgColor}`}>
            <div className="flex items-center justify-between mb-2">
              <span className={`text-sm font-medium ${isDarkMode ? 'text-white/80' : 'text-slate-900'}`}>{t.airQualityIndex}</span>
              <div className="flex items-center gap-2">
                {/* Pastille de couleur */}
                <div
                ...
                />
                <span className={`text-2xl font-bold ${isDarkMode ? aqiInfo.color : 'text-slate-900'}`}>
                  {airQualityData.current.us_aqi}
                </span>
              </div>
            </div>
            <p className={`text-sm font-medium ${isDarkMode ? aqiInfo.color : 'text-slate-900'}`}>
              {getAQILabelTranslated(airQualityData.current.us_aqi)}
            </p>
            <p className={`text-xs mt-1 ${isDarkMode ? 'text-white/60' : 'text-slate-600'}`}>
              {getAQIDescriptionTranslated(airQualityData.current.us_aqi)}
            </p>
            <div className={`grid grid-cols-2 gap-2 mt-3 pt-3 border-t ${isDarkMode ? 'border-white/10' : 'border-slate-200'}`}>
              <div>
                <p className={`text-xs ${isDarkMode ? 'text-white/60' : 'text-slate-600'}`}>PM2.5</p>
                <p className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                  {airQualityData.current.pm2_5.toFixed(1)} μg/m³
                </p>
              </div>
              <div>
                <p className={`text-xs ${isDarkMode ? 'text-white/60' : 'text-slate-600'}`}>NO₂</p>
                <p className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
